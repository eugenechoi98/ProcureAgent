...
`procureguard/productization/manual_audit.py` 定义手动输入请求、来源标签和请求级内存 SQLite adapter。它只把产品输入转换成现有 `ExtractedFields` 和 mock 表结构，再调用 `AgentInvoiceProcessor`；不复制三单匹配、Risk Engine 或解释逻辑，也不写默认应用数据库。

`procureguard/productization/manual_audit_store.py` 在 API 进程内保存 Phase 4C 请求/响应和 review metadata，并生成 JSON/Markdown export。review decision 是 additive metadata，不修改原始 deterministic risk/action；store 不持久化且不替代 Phase 2 正式数据库。
...
- `procureguard.phase3.explanation.structured_output`：Phase 3J 独立结构化解释 schema、fail-closed validator、确定性 renderer 和模板 fallback，不接 API。
- `procureguard.phase3.structured_evaluation`：运行离线 challenge set，输出字段级指标、拒绝原因和逐案例 debug rows。
- `procureguard.phase3.explanation.evidence_citation`：Phase 3K 从 Canonical Audit Facts 构造稳定 Evidence Catalog，并对 claim type、关键实体和 citation 覆盖做 fail-closed 校验。
- `procureguard.phase3.citation_evaluation`：分别统计 accepted-only 与 all-candidate citation 指标，并输出逐案例拒绝证据。
...
Phase 3H/4H 将解释层改为受控架构：Phase 2 输出 Canonical Audit Facts，确定性模板渲染器作为 MVP 官方解释输出，LoRA 只可作为 `guarded_lora` 或 `shadow_lora` controlled rewrite candidate。任何 LLM 输出必须通过 guard；失败、不可用、空输出、解析失败或高风险场景都回退到模板。

## Product MVP 目标架构

当前目标不是只展示离线 artifacts，也不是直接建设企业级采购系统。后续开源 MVP 按以下路径演进：

```mermaid
flowchart LR
    A["Invoice Image"] --> B["OCR tokens + bbox"]
    B --> C["LayoutLMv3 field candidates"]
    C --> D["Human confirmation / correction"]
    D --> E["ExtractedFields"]
    E --> F["Phase 2 deterministic audit"]
    F --> G["AuditReport + Canonical Facts"]
    G --> H["Deterministic template"]
    H --> I["Optional LoRA controlled rewrite"]
    I --> J["Guard"]
    J -->|PASS| K["Enhanced explanation"]
    J -->|FAIL| L["Template fallback"]
```

- OCR 是 token、bbox 和置信度输入来源，不替代 LayoutLMv3 字段抽取。
- LayoutLMv3 只输出字段候选，不决定风险、动作或异常类型。
- 字段候选经用户确认后才进入现有 Phase 2 主链，避免把抽取错误误判为采购风险。
- `risk_level`、`recommended_action` 和 `anomaly_types` 始终由确定性规则生成。
- deterministic template 是默认解释；LoRA 仅是显式启用的受控改写候选。
- LangChain Policy RAG comparison 是 Phase 4I optional integration，不能替代正式 SQLite FTS5/BM25 主链。
- SQLite persistence 在 Phase 4G 后按实际工作区需求评估；Docker hardening 属于 Phase 4J；HF live inference 需先通过本地资源和质量门槛；auth/multi-tenant 不在当前 Phase 4。

### Phase 4F 本地 Extraction Spike

`procureguard.extraction.live_spike` 是独立于 API 和 Phase 2 的本地适配层。它先只读检查微调 Safetensors checkpoint、processor、BIO label map、OCR 依赖和 CPU/GPU 策略，再按 `image -> PaddleOCR tokens/bbox -> LayoutLMv3 word labels -> field reconstruction -> candidates` 输出证据。

- 不导入 `AgentInvoiceProcessor`，不产生 AuditReport、风险或建议动作。
- Hugging Face/Transformers 强制 local-only，资产检查和推理入口都不静默下载模型。
- confidence 仅来自真实 word-label softmax；无 span 时为 `null`，所有字段强制人工确认。
- 任何资产、图片、OCR、推理或重建失败都写 machine-readable failure，并停止链路。
- Phase 4F 初始状态曾缺 Phase 1 微调 checkpoint、saved processor 和 BIO label map；Phase 4F.2 已从本地外部 artifact 恢复 runtime bundle，并完成一次真实 CPU live extraction。

`scripts/phase4/prepare_layoutlmv3_runtime_assets.py` 负责把外部找回的 Phase 1 checkpoint 整理到 ignored `artifacts/phase1_runtime/`。它校验 Safetensors、checkpoint config 和 BIO label order，随后复制匹配 processor、生成显式 label map，并用 manifest 记录 size/SHA256/source。缺 checkpoint 或 label order 不一致时保持 blocked，不允许用 base model 或代码标签定义拼出未经证明的 ready bundle。

Phase 4F.2 新增 `scripts/phase4/recover_or_rebuild_layoutlmv3_assets.py`，负责优先查找 `layoutlmv3_best.zip`、解压恢复 runtime bundle，并在 artifact 缺失时只生成 rebuild plan。当前已从本地外部 artifact 恢复 bundle，并用单张公开 SROIE 样例跑通 `OCR -> LayoutLMv3 field candidates`；该结果仍停在抽取层，不调用 Phase 2。

### Phase 4G 字段确认层

`procureguard.productization.field_confirmation` 将 LayoutLMv3 candidates、confidence、token span 和 bbox 转换为人工确认后的 `ConfirmedAuditInput`。critical fields 包括 `invoice_number`、`total_amount`、`vendor_name` 和 `invoice_date`，即使模型高置信也必须确认。

`POST /api/fields/confirm` 是最小 API：它返回 confirmed/corrected/rejected/missing fields 和 governance trace，但不调用 Phase 2、不生成 AuditReport、不生成 risk/action。后续 Phase 2 集成只能通过 `confirmed_audit_input_to_extracted_fields()` 从 `ConfirmedAuditInput.confirmed_fields` 读取事实字段，不能读取 raw model candidates。

### Phase 4G-EXT 端到端编排

`procureguard.productization.e2e_audit` 提供 `/api/mvp/audit/execute` 的业务编排。它支持三类输入：image、field candidates 和 confirmed fields。image 会先进入 Phase 4F live extraction；field candidates 进入 Phase 4G confirmation；confirmed fields 可直接作为 canonical truth。Phase 2 入口只接收 `ConfirmedAuditInput.confirmed_fields` 或已声明的 confirmed fields，不接 raw OCR 或 raw LayoutLMv3 output。

输出包含 AuditReport JSON、Markdown 和 trace。trace 记录 OCR/LayoutLMv3 是否使用、字段确认者、修改字段、deterministic rules 来源，以及 LoRA/Guard 是否仅作为解释层运行。
...
- Phase 3J 采用 rule-only Structured Output First baseline：模型不参与，结构化候选必须与 Canonical Audit Facts 的风险、动作、异常和缺失字段完全一致，每条 bullet 必须绑定允许 evidence ID；失败继续回退 Phase 3H 确定性模板。
- Phase 3K 在 Phase 3J 之后增加 evidence citation 离线验证；引用存在不代表语义必然正确，当前仅用保守规则检查声明类型、实体值和证据覆盖，正式路径仍为 Phase 3H deterministic template。
...
- Phase 4H 新增产品模式：`template` 不调用 provider；`guarded_lora` 仅在 Guard PASS 后使用 LoRA rewrite；`shadow_lora` 始终返回 template，只记录候选和 Guard 结果。旧 `shadow/experimental` 保持兼容。
- Audit Trail 记录 requested/used mode、final source、facts hash、template hash、LoRA candidate hash、template/guard/prompt version、provider/model/adapter、latency、raw output 保存状态、verifier result、fallback reason 和 final explanation。
- Phase 3H.2 在 `AgentInvoiceProcessor` 完成 Phase 2 风险与动作判断、构建 `AuditReport` 后调用解释层；解释结果不能写回 matcher、Risk Engine、工具调用或人工审核决策。
- `AuditReport.explanation` 是向后兼容的可选字段，API 默认 `explanation_mode=template`。`guarded_lora/shadow_lora` 必须显式选择并注入 provider；clean clone 没有真实 LoRA adapter，provider unavailable 时返回 template fallback。
