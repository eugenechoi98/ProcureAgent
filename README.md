...
ProcureGuard AI 是一个企业采购发票智能审核 Agent。Phase 1 模型抽取与 Phase 2 确定性审核链已封板，当前进入 Phase 3H 受控解释层阶段。
...
Phase 3H 已合并到 `main`。当前进入 Demo Deployability Review，只评估本地离线演示准备情况，不代表线上部署通过。首次公开 Demo 推荐优先使用固定或预生成 `ExtractedFields` 的混合模式，固定样例作为 fallback；完整在线 LayoutLMv3 和真实 LoRA 暂不作为首次部署 blocker。评估见 [DEMO_DEPLOYABILITY_REVIEW.md](docs/DEMO_DEPLOYABILITY_REVIEW.md)。
...
本地地址：`http://127.0.0.1:7860`。默认不 share、不创建公网链接、不需要 API Key 或 GPU。shadow 和 experimental 只使用 fake provider，真实 LoRA 与在线 LayoutLMv3 均未启用。详细说明见 [LOCAL_GRADIO_DEMO.md](docs/LOCAL_GRADIO_DEMO.md)。
