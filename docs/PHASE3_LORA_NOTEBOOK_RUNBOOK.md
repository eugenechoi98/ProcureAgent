...
- `preflight_ready`：数据 SHA、项目依赖、LoRA 依赖、模型目录和 Kernel Python 通过。
- `training_ready`：在 preflight 基础上，无论 `RUN_TRAINING` 当前是否为 `False`，都提前检查 CUDA、CUDA device count 和 bitsandbytes 4-bit 训练路径。

只有 `training_ready=True` 才允许把 `RUN_TRAINING=True`。失败时会在 `failed_checks` 中列出具体原因。

## 5. CUDA Runtime 兼容方案

Phase 3D 固定使用 CUDA 11.8 PyTorch wheel：
