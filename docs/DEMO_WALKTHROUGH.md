...
1. 先选择案例 A，查看同一 SROIE sample_id 的原图、OCR 框、离线 LayoutLMv3 预测和 Phase 2 审核结果。
2. 切换案例 B 查看日期 span 清理与重建；这是单样本证据，整体 Date F1 仍以模型实验页为准。
3. 切换案例 C 查看真实离线 LoRA 输出、Guard `REJECT` 和确定性模板 fallback。
4. 原 5 个合成流程案例保留在默认收起的补充区，需要演示交互式审核时再展开。
3. 保持默认解释模式，或在供应商不一致/高风险案例中查看 Guard / fallback 演示。
4. 点击“运行审核”，查看风险等级、建议动作和审核解释。
5. 如需核查结构化字段，展开“查看完整技术输出”。

这一页把图片、字段预测、审核结果和解释治理串成可追溯证据链。A/B 的 PO/GRN 是明确标注的 mock 采购上下文；C 的输入事实是 synthetic evaluation fixture。最终风险等级与建议动作仍由确定性规则事实决定，不由 LoRA 改写。
